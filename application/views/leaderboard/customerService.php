...
<head>
  ...
  <style>
    ...
    main {
      ...
      background: url("https://images.unsplash.com/photo-1578683010236-d716f9a3f461?auto=format&fit=crop&w=1350&q=80") center/cover no-repeat;
      position: relative;
    }
...
<body>
  ...
  <script>
    ...
    // getFloor
    $.ajax({
      url: '<?= base_url() ?>Cleaning/getFloor',
      type: 'POST',
      success: (response) => {
        $('#floorSelect').append('')
        var res = JSON.parse(response)
        var option = '<option selected disabled>🏢 Pilih Lantai</option>'
        $.each(res.msg, (key, val) => {
    ...
    //getKamar
    $('#floorSelect').on('change', (e) => {
      e.preventDefault()
      $.ajax({
        url: '<?= base_url() ?>Cleaning/getRoomsByFloor',
        type: 'POST',
        data: {
        ...
        success: (response) => {
          ...
          $('#roomSelect').append('')
          var res = JSON.parse(response)
          var option = '<option selected disabled>🚪 Pilih Nomor Kamar</option>'
          $.each(res.msg, (key, val) => {
            option += `<option value="` + val.ID + `">` + val.NAMA + `</option>`
          })
